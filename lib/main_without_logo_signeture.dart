...
class _IDCardScreenState extends State<IDCardScreen> {
  ...
  Future<void> _fetchData(String empId) async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse('$baseUrl/api/HR/Person/IdentityCard?id=$empId&macid=$deviceId'));
      if (response.statusCode == 200) {
        var rawData = jsonDecode(response.body);

        // Mac ID Validation
        String remoteMac = rawData["card"]["macId"] ?? "";
        if (remoteMac != "" && remoteMac != deviceId ) {
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            onSelected: (val) => val == 'refresh' ? _fetchData(employeeData!['company_digital_id']['front_side']['employee_id'].split('-').last) : _handleLogout(),
            itemBuilder: (context) => [
